...
template <typename T> Array<T> &Array<T>::operator=(const Array<T> &source) {
    ...
    } else // not the same object
    {
        ...
        for (int i = 0; i < size;
             i++) // initialize every element this->data with the corresponding element in source.data
        {
...
template <typename T> T &Array<T>::operator[](int index) {
    if (index > size - 1 || index < 0) // check index is out of bound
    {
...
template <typename T>
const T &Array<T>::operator[](int index) const // This is used to handle the index
{
    if (index > size - 1 || index < 0) // check index is out of bound
    {
        throw OutOfBoundsException(index);
...
template <typename T> T &Array<T>::GetElement(int i) const {
    if (i > size - 1 || i < 0) // check index is out of bound
    {
...
// Setter
template <typename T> void Array<T>::SetElement(int &i,const T &t) {
    if (i >= 0 && i < size) // check index is inside the bound
    {
        data[i] = t;
