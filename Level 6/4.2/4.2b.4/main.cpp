...
int main() {
    ...
    // for (int i = 0; i != 16; i++) { // This will be out of bounds
    for (int i = 0; i != 15; i++) {
        try {
            intstk1.Push(i + 1);
            cout << "Index = " << intstk1.GetCurrent() << "  Element:" << intstk1.GetEle()
        ...
        } catch (ArrayException &err1) // handle out-of-bound excpetion
        {
            ...
            cout << "The last one on stack is: " << last << "  with iterator:" << i << endl;
    ...
    for (int i = 0; i != 15; i++) {
        try {
            ...
            cout << "The last one on stack is: " << lastele << "  with iterator:" << i << endl;
        } catch (ArrayException &err2) // handle out-of-bound excpetion
        {
            cout << err2.GetMessage() << endl;
            intstk1.Push(i + 1); // add an element
            cout << "Index = " << intstk1.GetCurrent() << "  Element:" << intstk1.GetEle()
                 << "  with iterator:" << i << endl; // get a clearer version(for debugging)
        } catch (...) // respond to unknow error
        {
            cout << "Unknown Error!" << endl;
        }
    ...
    for (int i = 0; i != 7; i++) {
        try {
            intstk2.Push(i + 1);
            cout << "Index = " << intstk2.GetCurrent() << "  Element:" << intstk2.GetEle()
                 << "  with iterator:" << i << endl;
        } catch (ArrayException &err1) // handle out-of-bound excpetion
        {
            cout << err1.GetMessage() << endl;
