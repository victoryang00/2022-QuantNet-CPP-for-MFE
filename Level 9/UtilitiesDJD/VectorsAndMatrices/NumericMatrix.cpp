...
{ // Return row. Overloads Matrix::Row() to return Vector instead of Array
	...
	Vector<V,I> result(Columns<V,I,S>(), MinColumnIndex<V,I,S>());
...
//	Matrix<V, I, S>::Row(row, val);
	...
	for (I c=MinColumnIndex<V,I,S>(); c<=MaxColumnIndex<V,I,S>(); c++)
	{
...
{ // Unary minus
	...
	NumericMatrix<V, I, S> result(Rows<V,I,S>(), Columns<V,I,S>(), MinRowIndex<V,I,S>(), MinColumnIndex<V,I,S>());

	// Copy all elements negative
	for (I r=MinRowIndex<V,I,S>(); r<=MaxRowIndex<V,I,S>(); r++)
	{
		for (I c=MinColumnIndex<V,I,S>(); c<=MaxColumnIndex<V,I,S>(); c++)
		{
...
{ // Add the elements
	...
	// Add all elements
	for (I r1=MinRowIndex<V,I,S>(); r1<=MaxRowIndex<V,I,S>(); r1++)
	{
		for (I c1=MinColumnIndex<V,I,S>(); c1<=MaxColumnIndex<V,I,S>(); c1++)
...
{ // Multiply the matrix.
  ...
  // using the InnerProduct function of Vector. This saves a couple of row and column copies.
	...
	NumericMatrix<V, I, S> result(Rows<V,I,S>(), m.Columns(), MinRowIndex<V,I,S>(), MinColumnIndex<V,I,S>());
	I r1, c2, rr, cr;
	...
	for (I rr=result.MinRowIndex(); rr<=result.MaxRowIndex(); ++rr)
	{
		...
		for (I cr = result.MinColumnIndex(); cr <= result.MaxColumnIndex(); ++cr) 
		{
			...
			std::cout << "*" << rr << "," << cr << "," << result(rr, cr)<< std::endl;
		}
...
template <class V, class I, class S> Vector<V, I> NumericMatrix<V, I, S>::operator * (const Vector<V, I>& v) const
{
	...
	Vector<V, I> result(Rows<V,I,S>(), v.MinIndex());

	V r(0.0);	// sum of rows
	for (I i = MinRowIndex<V,I,S>(); i <= MaxRowIndex<V,I,S>(); i++)
	{
		r = 0.0;
		for (I j = MinColumnIndex<V,I,S>(); j<= MaxColumnIndex<V,I,S>(); j++)
		{
				r += (*this)(i,j) * v[j];
		}
		result[i] = r;
...
{ // Switch rows and columns
		...
		NumericMatrix<V, I, S> result(Columns<V,I,S>(), Rows<V,I,S>());
